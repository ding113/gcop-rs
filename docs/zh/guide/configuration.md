...
lockfile_patterns = ["**/*.lock"]  # 额外 lockfile pattern；发送给 LLM 前只保留摘要
...
| `lockfile_patterns` | Array | `[]` | 额外的依赖 lockfile glob pattern；匹配文件的完整 diff 永远不会发送给 LLM |

常见依赖 lockfile 已内置，commit/review/hook prompt 中始终只发送摘要，包括 `Cargo.lock`、`package-lock.json`、`npm-shrinkwrap.json`、`yarn.lock`、`pnpm-lock.yaml`、`poetry.lock`、`Pipfile.lock`、`uv.lock`、`composer.lock`、`Gemfile.lock`、`go.sum`、`go.work.sum`、`bun.lockb`、`bun.lock`、`deno.lock`、`flake.lock`、`conan.lock`、`pubspec.lock`、`mix.lock`、`stack.yaml.lock` 和 `Podfile.lock`。

lockfile 发生变更时，gcop-rs 仍会保留文件名和变更量，例如 `Cargo.lock (+42 -7) [lockfile]`；只省略完整 patch 内容。只有 lockfile 一个文件变更时也是这个行为。
